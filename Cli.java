...
public class Cli {
    ...
    static void register() {
        ...
        try {
            ...
            PreparedStatement stmt = conn.prepareStatement(
                "INSERT INTO Users (username, password, hp, role, ownerNIK) VALUES (?, ?, ?, ?, ?)"
            );
    ...
    static void login() {
        ...
        try {
            PreparedStatement stmt = conn.prepareStatement(
                "SELECT * FROM Users WHERE username = ? AND hp = ? AND password = ?"
            );
            ...
            if (rs.next()) {
                ...
                if (otp.equals(inputOtp)) {
                    ...
                    log("Login sebagai " + currentUser.role + " (" + currentUser.username + ")");
                    showUserMenu();
                    showMainMenu(); // Return to main menu after user menu exits
                } else {
    ...
    static void showUserMenu() {
        ...
        while (running) {
            ...
            running = switch (currentUser.role) {
    ...
    static boolean pemilikMenu() {
        ...
        switch (ch) {
            case 1:
                laporanPemakaianPemilik();
                return true;
            case 2:
                kendalikanAirPemilik();
                return true;
            case 0:
                exit();
                return false;
